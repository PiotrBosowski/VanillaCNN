...
TEST(output_layer_check, constructing_test)
{
    //given
    ...
    //when
    FullyConnectedLayer_ dense(&previousLayer, numberOfNeurons);
    dense.populate();
    dense.connect();
...
TEST(output_layer_check, connecting_test_positive)
{
    //given
    ...
    //when then
    ASSERT_NO_THROW(
            FullyConnectedLayer_ testingLayer(&second_fully, 80);
            testingLayer.populate();            testingLayer.connect();

    );
    ASSERT_NO_THROW(
            FullyConnectedLayer_ testingLayer(&second_convo, 80);
            testingLayer.populate();            testingLayer.connect();
    );
    ASSERT_NO_THROW(
            FullyConnectedLayer_ testingLayer(&second_downs, 80);
            testingLayer.populate();
            testingLayer.connect();
    );
    ASSERT_NO_THROW(
            FullyConnectedLayer_ testingLayer(&inputLayer, 80);
            testingLayer.populate();            testingLayer.connect();
    );
    ASSERT_NO_THROW(
            FullyConnectedLayer_ testingLayer(&second_output, 80);
            testingLayer.populate();
            testingLayer.connect();
    );
}
